...
import com.quanxiaoha.airobot.constant.DateConstants;
import com.quanxiaoha.airobot.utils.JsonUtil;
...
/**
 * ObjectMapper 就是 Jackson 用来处理 JSON 的核心工具，自定义它的配置，
 * 是为了让项目中所有 JSON 处理（接口、工具类）都遵循统一的规则，避免格式不一致的问题。
 * JacksonConfig 是SpringBoot自定义JSON序列化/反序列化规则的配置类,用于核心解决以下问题:
 * 1.前端传的json和后端实体类字段不匹配(少传了字段)
 * 2.日期类型 (LocalDateTime等) 序列化格式不一致,时区混乱
 * 3.空值字段返回给前端,影响接口规范
 */
@Configuration
public class JacksonConfig {
    ...
    public ObjectMapper objectMapper() {
        ...
        /**
         * 把你在 JacksonConfig 里配置好的 ObjectMapper 实例，传给 JsonUtil，让工具类里也用这个带自定义配置的实例，而不是自己 new 一个。
         */
